...
/**
 * EcTSmBusbarStationary: steady electric -> thermal -> structural coupling on
 * an L-shaped copper busbar carrying three titanium bolts.
 *
 * <p>Geometry: an L-shaped cross-section in the xz plane — the outer rectangle
 * (L+2*tbb) x Hbb minus the inner rectangle (L+tbb) x (Hbb-tbb) placed at
 * (0, tbb) — with the inner corner filleted by tbb and the outer corner by
 * 2*tbb, extruded by wbb along y. Three bolts of radius rad_1 pierce the
 * plate: one through the vertical leg along +x, two through the horizontal
 * leg along +z. Each bolt reaches through the tbb wall and protrudes 2*tbb
 * beyond it, so the bolt's inner end face is flush with the plate surface and
 * the plate surface around it stays exterior. Form Union keeps the internal
 * boundaries, which leaves 7 domains: the busbar, plus two segments (through
 * the wall, and outside it) per bolt.
 *
 * <p>Materials: Copper on the busbar and Titanium beta-21S on the six bolt
 * segments. The assignment is per domain, so every domain carries a material
 * and no coefficient is left at the zero the app starts from.
 *
 * <p>Boundaries: ec — the free end face of the vertical bolt is the voltage
 * terminal (V = Vtot), the free end faces of the two horizontal bolts are
 * grounded, every other exterior face is insulated. ht — natural convection
 * (htc towards T0) on every exterior face except those three, which stay
 * insulated. solid — those three faces are fixed. Couplings:
 * ElectromagneticHeating (ec -> ht) and ThermalExpansion (ht -> solid) from
 * T0. The study is stationary: the case is here for the mesh, the materials
 * and the boundary set, and a time integration would only add the cost of
 * re-solving those.
 *
 * <p>Verification points (the model has no closed-form solution: the two
 * materials, the L-shaped current path and the bolt contact faces all act, and
 * the reference is the COMSOL solution). The titanium bolts dominate the
 * circuit. One bolt has the section pi*rad_1^2 and the length 3*tbb, so
 * R_bolt = 3*tbb/(sigma_Ti*pi*rad_1^2) = 1.79e-4 Ohm, the two grounded bolts
 * in parallel plus the terminal bolt give 2.69e-4 Ohm, and I = Vtot/R = 74 A at
 * Vtot = 20 mV. The copper path contributes under a percent (its resistance is
 * of order L/(sigma_Cu*tbb*wbb) = 1.1e-5 Ohm, plus the L-bend), so the terminal
 * voltage drops almost entirely across the bolts: 13.3 mV over the terminal
 * bolt and 6.7 mV over each grounded one — the reference potential runs from 0
 * to Vtot across the three, while the whole plate carries only the copper drop,
 * 6.42 to 7.13 mV over its entire extent.
 *
 * <p>That 1.5 W of Joule heat leaves through the exterior faces at
 * htc = 5 W/(m^2*K), and copper spreads it: the reference puts the plate at a
 * nearly uniform 16.8 K above T0 (its whole spread is 0.3 K over 3228 nodes),
 * with the bolt material reaching 25.0 K where it carries the current into the
 * plate. The balance h*A*<dT> = 5*0.022*17.5 = 1.9 W reproduces the dissipated
 * power to the accuracy of that area estimate. The clamped bolt ends hold the
 * bolt axes, so the displacement field is the thermal expansion of the plate:
 * alpha_Cu*17.5 K = 3.0e-4 relative over the 10 cm leg, and the reference's
 * largest displacement magnitude is 22 µm.
 *
 ...
 * it borders lies on one of the three bolt cylinders; the remaining domain is
 * the busbar. An exterior face is a bolt end face when all of its sampled
 * points sit at one axial end of that cylinder, within its radius. The
 * terminal is the free end of the vertical bolt, the ground is the free ends
 * of the two horizontal bolts, and the faces left over carry the convection.
 * The three sets are asserted: a face that went unclassified would silently
 * drop a boundary condition, which no comparison of the result can localize.
 *
 * <p>Export: args[0]=result.txt (V, T, solid.disp), args[1]=mesh.mphtxt,
...
public class EcTSmBusbarStationary {
    ...
    public static void main(String[] args) throws Exception {
        ...
        final String[] P = a.length >= 4 ? a
            : new String[]{"result.txt", "mesh.mphtxt", "EcTSmBusbarStationary.mph",
        ...
        model.save(P[3], "java");
        System.out.println("EcTSmBusbarStationary_OK");
    }
