...
/**
 * TBlockNonlinTransient: transient heat conduction with a temperature
 * dependent conductivity.
 *
 * Geometry: 3D Block (0.1 x 0.05 x 0.01 m), single domain.
 * Physics: ht
 *   - initial temperature Tini (the block starts hot),
 *   - both x faces held at T0, so the block cools while the conductivity
 *     follows the temperature.
 * Material: k(T) = k0 (1 + alpha_k (T - Tref)) with a constant volumetric
 *   heat capacity; the parameter names must not collide with the material's
 *   own property variables (k, rho, Cp).
 * Study: Transient, tlist = range(0, dtout, tend).
 */
public class TBlockNonlinTransient {

    public static void main(String[] args) throws Exception {
        String[] a = args == null ? new String[0] : args;
        final String[] P = a.length >= 4 ? a
            : new String[]{"result.txt", "mesh.mphtxt", "TBlockNonlinTransient.mph", "generated_model.java"};
        ...
        model.param().set("k0", "45[W/(m*K)]", "conductivity at Tref");
        model.param().set("alpha_k", "-0.005[1/K]", "conductivity slope");
        ...
        // Material: temperature dependent conductivity
        model.component(comp).material().create("mat1", "Common");
        model.component(comp).material("mat1").label("NonlinSteel");
        ...
        model.save(P[3], "java");
        System.out.println("TBlockNonlinTransient_OK");
    }
}
