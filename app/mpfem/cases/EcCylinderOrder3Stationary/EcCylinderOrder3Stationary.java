...
/**
 * EcCylinderOrder3Stationary: 3D 空心圆柱导电稳态, 二阶几何 + 三阶单元。
 *
 * <p>几何: 外圆柱 r2 减去内圆柱 r1, 高 hcyl。内、外表面都是曲面, 因此 COMSOL
 * 的 Mesh 导出写二阶几何单元 (edg2/tri2/tet2), 即等参 P2 几何。网格刻意取粗
 * (mh 大), 让单元阶次的差别在结果上明显可见。
 *
 * <p>物理: ec (内表面 Terminal V=V0, 外表面 Ground, 端面默认绝缘)。
 * 单元阶次由模型自己指定: physics("ec").prop("ShapeProperty")
 * .set("order_electricpotential", "3") —— 三阶, 与几何阶次 (2) 无关。
 *
 * <p>解析参照 (验证要点): 端面绝缘时电位只沿径向变化,
 *       V(r) = V0 * ln(r/r2) / ln(r1/r2)
 * 于是同一套网格上提高单元阶次必须单调逼近这个解; 若程序忽略模型声明的阶次
 * (退化成按几何阶次取 2), 结果就不会随阶次变化。
 *
 * <p>导出: args[0]=result.txt (V), args[1]=mesh.mphtxt, args[2]=.mph,
 *       args[3]=generated_model.java (Save-As-Java)。
 */
public class EcCylinderOrder3Stationary {

    public static void main(String[] args) throws Exception {
        String[] a = args == null ? new String[0] : args;
        final String[] P = a.length >= 4 ? a
            : new String[]{"result.txt", "mesh.mphtxt", "EcCylinderOrder3Stationary.mph",
        ...
        model.param().set("mh", "0.006[m]", "最大网格尺寸");
        ...
        // ---- 物理场: 单元阶次由模型声明, 与几何阶次无关 ----
        model.component(comp).physics().create("ec", "ConductiveMedia", "geom1");
        model.component(comp).physics("ec").prop("ShapeProperty")
            .set("order_electricpotential", "3");
        ...
        model.save(P[3], "java");
        System.out.println("EcCylinderOrder3Stationary_OK");
    }
