...
namespace {
    ...
    std::array<std::vector<T>, 2> rec_jacobi(int N, T a, T b)
    {
        ...
        std::ranges::transform(n, nab.begin(), [a, b](auto x) { return static_cast<T>(2.0 * x + a + b); });

        std::vector<T> alpha(N);
        alpha.front() = nu;
        std::ranges::transform(
            nab, std::next(alpha.begin()), [a, b](auto nab) { return static_cast<T>((b * b - a * a) / (nab * (nab + 2.0))); });
    ...
    std::vector<T> compute_gauss_jacobi_points(T a, int m)
    {
        ...
        for (int k = 0; k < m; ++k) {
            // Initial guess
            x[k] = -std::cos((2.0 * k + 1.0) * std::numbers::pi / (2.0 * m));
            if (k > 0)
                x[k] = 0.5 * (x[k] + x[k - 1]);
    ...
    std::array<std::vector<T>, 2> make_quadrature_line(int m)
    {
        ...
        std::ranges::transform(ptx, ptx.begin(), [](auto x) { return static_cast<T>(0.5 * (x + 1.0)); });
