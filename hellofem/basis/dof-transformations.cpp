...
namespace {
    ...
    mapinfo_t<T> get_mapinfo(cell::type cell_type)
    {
        ...
        case cell::type::hexahedron: {
            ...
            {
                auto& data = mapinfo.try_emplace(cell::type::interval).first->second;
                auto map = [](auto pt) -> std::array<T, 3> { return {1 - pt[0], pt[1], pt[2]}; };
                mdex::mdarray<T, md::extents<std::size_t, 3, 3>> J(
                    md::extents<std::size_t, 3, 3> {},
                    {-1., 0., 0., 0., 1., 0., 0., 0., 1.});
...

            {
                ...
                {
                    auto map = [](auto pt) -> std::array<T, 3> { return {1 - pt[1], pt[0], pt[2]}; };
                    mdex::mdarray<T, md::extents<std::size_t, 3, 3>> J(
                        md::extents<std::size_t, 3, 3> {},
                        {0., 1., 0., -1., 0., 0., 0., 0., 1.});
        ...
        case cell::type::prism: {
            ...
            }
            {
                auto& data = mapinfo.try_emplace(cell::type::triangle).first->second;
                {
                    auto map = [](auto pt) -> std::array<T, 3> { return {1 - pt[1] - pt[0], pt[0], pt[2]}; };
                    mdex::mdarray<T, md::extents<std::size_t, 3, 3>> J(
                        md::extents<std::size_t, 3, 3> {},
                        {0., 1., 0., -1., -1., 0., 0., 0., 1.});
                    T detJ = 1.0;
            ...
            }
            {
                auto& data = mapinfo.try_emplace(cell::type::quadrilateral).first->second;
                {
                    auto map = [](auto pt) -> std::array<T, 3> { return {1 - pt[2], pt[1], pt[0]}; };
                    mdex::mdarray<T, md::extents<std::size_t, 3, 3>> J(
                        md::extents<std::size_t, 3, 3> {},
                        {0., 0., 1., 0., 1., 0., -1., 0., 0.});
                    T detJ = 1.0;
        ...
        case cell::type::pyramid: {
...

            {
                auto& data = mapinfo.try_emplace(cell::type::triangle).first->second;
                {
                    auto map = [](auto pt) -> std::array<T, 3> { return {1 - pt[2] - pt[0], pt[1], pt[0]}; };
                    mdex::mdarray<T, md::extents<std::size_t, 3, 3>> J(
                        md::extents<std::size_t, 3, 3> {},
                        {0., 0., 1., 0., 1., 0., -1., 0., -1.});
                    T detJ = 1.;
